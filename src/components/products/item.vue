<template>
  <dl class="s-item">
    ...
    <dd>
      ...
      <p>
        <el-rate v-model="meta.score" disabled text-color="#ff9900" />
      ...
      <ul class="deal-items">
        <li class="items" v-for="(item, index) in meta.dealItems.slice(0,2)" :key="index">
          ...
          <p class>
            <span class="deal-price">{{item.price}}{{item.priceType}}</span>
            <span class="deal-old-price">{{item.counterPrice}}{{item.priceType}}</span>
